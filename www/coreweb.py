...
# 若不编写此函数来映射一个URL的处理，那么就要：
# 1.对处理的函数添加async修饰
# 2.从传入的request中自己提前参数
# 3.自己构造response对象返回
'''
使用方法，在指定URL的处理函数上使用装饰 
@get('/api/comments')
...
def get(path):
    ...
    '''
    def decorator(func):
        @functools.wraps(func) # 将原函数func的__name__复制给新函数wrapper
        def wrapper(*args, **kw): # 装饰器
            return func(*args, **kw)
        ...
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
            if request.method == 'POST':
                ...
                if ct.startswith('application/json'):
                    ...
                    kw = dict(**params) # 可变关键字参数，此处用这方式初始化dict
                else:
...
# 这样注册过后，对URL函数进行统一处理，那就保证了函数都是coroutine，以及指定给 RequestHandler 来处理了常规的参数检查、解析获取等操作
