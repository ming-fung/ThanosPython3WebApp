...
#创建连接池，复用数据库，避免频繁关闭打开数据库 (asyncio.coroutine和await可使用async和await代替，便于理解）
...
async def select(sql, args, size=None):
    log(sql, args)
    global __pool
    async with __pool.get() as conn: #线程内异步获取pool，等收到获取到了pool的消失后，消息管理器回到上次发起异步的位置，返回from的内容
